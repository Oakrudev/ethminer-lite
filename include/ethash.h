namespace ethash {
	...
	const uint32_t DATASET_BYTES_INIT = 2**30        // bytes in dataset at genesis
	const uint32_t DATASET_BYTES_GROWTH = 2**23      // dataset growth per epoch
	const uint32_t CACHE_BYTES_INIT = 2**24          // bytes in cache at genesis
	const uint32_t CACHE_BYTES_GROWTH = 2**17        // cache growth per epoch
